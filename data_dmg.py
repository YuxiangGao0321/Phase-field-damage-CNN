...
class MaterialsDataset(Dataset):
    def __init__(self, data_dir, subsample=10000, split='all'):
        ...
        if not os.path.exists(examples_filename):
            ...
            for idx in range(len(examples)):
                # examples[idx]['split'] = 'test' if idx < 150 else 'train'
                if examples[idx]['volume_fraction'] == 0.3 and examples[idx]['case_idx'] > 500:
    ...
    def get_resampled_result(self,example):
        filename = os.path.join(example['dir'],'resampled_damage.th')
    ...
    def get_distance_field(self,example):
        filename = os.path.join(example['dir'],'distance_field.th')
        return torch.load(filename)
    ...
    def __getitem__(self,idx):
        ...
        distance_field = self.get_distance_field(example)
        # --- stress field
        stress_field = self.get_resampled_result(example)
